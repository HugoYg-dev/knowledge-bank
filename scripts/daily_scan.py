...
def generate_report(workspace_dir: Path) -> tuple[Path, str, str]:
    ...
    lines = [
        ...
        "",
        "## 📋 Vault Lint 健康摘要",
        f"- 强制检查状态：{'✅ 全部通过 (100%)' if lint_passed else '❌ 存在异常'}",
        f"- 死链：{lint_metrics['broken_links']} 处",
    ...
    if lint_metrics["low_freq_entities"] > 0:
        lines.append(f"- 提示项（L0 候选）：发现 {lint_metrics['low_freq_entities']} 个入度 ≤ 1 的低频实体（只进入候选报告）")
    ...
    lines.append("")
    lines.append("## ✅ 结论")
    if lint_passed and len(clippings_files) == 0 and review_count == 0 and unhandled_count == 0:
        conclusion = "全库图谱健康，无死链无格式错误，待处理队列已全部清空。"
    elif lint_passed:
        pending_items = []
        if clippings_files:
        ...
        conclusion = f"图谱结构健康（0死链/0错误）。当前待处理队列：{ '，'.join(pending_items) }。"
    else:
        conclusion = "图谱存在 Lint 错误，需优先排查修复！"
    ...
    summary_line = f"[Daily Scan] Lint: {'PASS' if lint_passed else 'FAIL'} | Clippings: {len(clippings_files)} | Review: {review_count} | Unhandled: {unhandled_count} -> 已写入 {report_file.name}"
