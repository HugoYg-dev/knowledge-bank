...
# 概念_Self_Harness_自主进化宿主系统

## 定义

**Self-Harness（自主外壳进化闭环）** 是一种将 [[concepts/概念_RSI递归自我改进|RSI（Recursive Self-Improvement）]] 落地为严格工程实践的范式。它让 Agent 系统在不更改模型权重的前提下，自动诊断自身在长程任务中的弱点并重构 [[concepts/概念_Harness_Engineering_宿主编排工程|Harness]] 宿主系统代码或在运行时动态替换控制流组件。
...
- **相关概念**：[[concepts/概念_Harness_Engineering_宿主编排工程|Harness Engineering]]、[[concepts/概念_Loop_Engineering循环工程|Loop Engineering 循环工程]]、[[concepts/概念_RSI递归自我改进|RSI 递归自我改进]]
