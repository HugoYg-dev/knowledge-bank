...
**Delta Attention（增量注意力）** 是 Kimi K3 模型采用的一种新型注意力与上下文检索优化机制。它通过将不断增长的历史 KV 信息压缩并折叠进一个固定大小的矩阵（Fixed-size matrix）中，从而消除传统自回归解码中随着序列无限增加的 `[[概念_KV_Cache_键值缓存]]` 存储，解决长文本推理中的内存膨胀和计算复杂度问题。
