...
## 核心要点
1. **复杂度瓶颈**：标准 [[wiki/concepts/概念_t_SNE_降维算法|t-SNE 算法]]的运行时间与样本数量呈二次方（quadratic，O(N²)）关系。当数据样本量超过 4 万（40k+）时，Sklearn 的标准 CPU 实现将变得非常缓慢和难以使用。
2. **GPU 加速解决方案 (tSNE-CUDA)**：tSNE-CUDA 是一种经过 CUDA 优化的 [[wiki/concepts/概念_t_SNE_降维算法|t-SNE 算法]]实现，在 CIFAR-10 数据集（5万张图像）上的基准测试中，相比 Sklearn 的 CPU 实现，运行速度提升了 700 倍。
