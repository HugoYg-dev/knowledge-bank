...
3. **缓存效益优于路由分流**：前缀缓存（Prefix Cache / [[concepts/概念_KV_Cache]]）的读取成本比无缓存输入便宜 75% 至 90%。System Prompt 和多轮对话历史构成了核心 Token 开销；若路由考虑缓存粘性则无法分流，若频繁分流则破坏缓存击穿，使得路由在成本优化上得不偿失。
