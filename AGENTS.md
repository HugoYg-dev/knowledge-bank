...
| `wiki/concepts/` | 概念页（严格遵循三层命名规范：`概念_{缩写/专名}_{中文释义}.md` 或 `概念_{中文规范名}.md`，详见 §2.3） |
...
- **路径与命名规范**：`wiki/concepts/概念_xxx.md`。全库所有概念页物理前缀一律为 `概念_`，使用**下划线 `_` 作为天然语义分界符**，并严格遵守以下**概念命名三分层权威规范**：
  1. **规则 1：英文缩写 + 中文核心名（业内通用首字母缩写）**
     - **格式**：`概念_{英文缩写}_{中文核心名}.md`
     - **范例**：`概念_PCA_主成分分析.md`、`概念_LoRA_低秩适应微调.md`、`概念_DPO_直接偏好优化.md`、`概念_GQA_分组查询注意力.md`、`概念_RLVR_可验证奖励强化学习.md`、`概念_BM25_最佳匹配25算法.md`
     - **约束**：英文缩写统一前置，采用大写或业界标准大小写；**严禁中英倒置**（❌ 如 `概念_主成分分析_PCA`）、**严禁纯英文缩写**（❌ 如 `概念_DPO`）、**严禁中英紧贴无下划线**（❌ 如 `概念_LoRA低秩适应微调`）。
  2. **规则 2：英文专名 + 中文核心名（专有算法 / 复合专有名词）**
     - **格式**：`概念_{英文专名}_{中文释义}.md`
     - **范例**：`概念_FlashAttention_快速注意力.md`、`概念_Fine_Tuning_模型微调.md`、`概念_KV_Cache_键值缓存.md`、`概念_Early_Stopping_早停机制.md`、`概念_Verifiable_Reward_可验证奖励机制.md`
  3. **规则 3：原生中文概念（无业界通用英文对标词汇）**
     - **格式**：`概念_{中文规范名}.md`
...
- **Frontmatter `aliases` 强约束与未链接提及机制**：
  ...
  - 英文全称（如有，如 `Contextualized Late Interaction over BERT`）
  - 中文核心名及常用简称（如 `多向量延迟交互检索`、`延迟交互`）
  - 历史旧文件名（若曾重命名更名，如 `概念_ColBERT`，保障向后兼容检索）
- **Frontmatter 示例**：
```yaml
---
type: "concept"
...
---
```
- **内容要求**：定义、核心机制与原理解析、使用场景 / 算法复杂度 / 选型权衡、关联概念与实体。
